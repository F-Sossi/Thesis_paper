...
\begin{itemize}
    \item \textbf{Traditional descriptors} such as SIFT~\cite{dong2015domain}, SURF, and their variants rely on hand-crafted gradient histograms computed at keypoint locations detected by scale-space analysis.

    \item \textbf{Learned descriptors} such as HardNet and SOSNet use convolutional neural networks trained on patch correspondence tasks, typically paired with learned detectors like KeyNet.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{DescriptorWorkbench Framework.} We develop an open-source evaluation framework implementing the image matching, keypoint verification, and keypoint retrieval metrics from Bojanic et al.~\cite{bojanic2020comparison}.
\end{enumerate}
