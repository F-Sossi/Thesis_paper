...
\begin{enumerate}
    ...
    \item \textbf{Complementary Descriptor Fusion}: Pairing descriptors with different strengths HoNC (high verification) with CNN (high matching)---yields improvements. HoNC + SOSNet achieves 50.6\% mAP on color patches, outperforming all individual descriptors.
...
\begin{enumerate}
    ...
    \item \textbf{For best full-image matching}: Use HardNet on detector intersection keypoints (82.1\% mAP), or HardNet+SOSNet concatenation (93.4\% mAP) for highest accuracy.
...
\begin{itemize}
    \item Oxford5k~\cite{oxford-dataset} and Paris6k~\cite{philbin2008lost} (image retrieval)
    \item MegaDepth~\cite{li2018megadepth} (wide-baseline matching)
    \item ETH3D~\cite{schops2017eth3d} (multi-view stereo)
\end{itemize}
