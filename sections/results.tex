...
CNN descriptors (HardNet, SOSNet) achieve the best single-descriptor matching performance. HoNC has low matching (18.5\%) but high verification (71.0\%), suggesting strong discriminative capability it rejects false matches well but struggles with invariance to viewpoint/illumination changes.
...
\caption{Patch benchmark matching mAP for baselines and fusions. Complementary fusions (discriminator + matcher) outperform all individual descriptors and same-family fusions, while pairing two weak matchers degrades performance below either component alone.}
...
\begin{enumerate}
    ...
    \item \textbf{Weak matcher fusion fails}: SIFT+HoNC (15.5\%) and SIFT+RGBSIFT (10.4\%) perform \textit{worse} than either component alone. To ensure equal contribution in all fusions, we apply pre-fusion L2 normalization to deal  magnitude mismatch. In this case SIFT (22.9\%) and HoNC (18.5\%) are both weak matchers compared the learned invariance of CNN descriptors. Concatenating two weak descriptors doubles dimensionality without adding additional information. This higher dimensionality actually hurts nearest-neighbor search, because the extra dimensions introduce noise rather than matching signal. In contrast, HoNC succeeds when paired with a strong matcher like SOSNet (50.6\%): the CNN provides robust matching, and HoNC's color information improves the matching performance.
...
\begin{itemize}
    \item CNN descriptors are trained on gray scale patches, learning edge and texture patterns
