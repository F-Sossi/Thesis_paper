...
\begin{enumerate}
    ...
    \item \textbf{Weak matcher fusion fails}: SIFT+HoNC (15.5\%) and SIFT+RGBSIFT (10.4\%) perform \textit{worse} than either component alone. Note that all fusions apply pre-fusion L2 normalization (ensuring equal contribution), so magnitude mismatch is not the cause. Rather, SIFT (22.9\%) and HoNC (18.5\%) are both weak matchers that lack the learned invariance of CNN descriptors. Concatenating two weak descriptors doubles dimensionality without adding robustness the increased dimensionality makes nearest-neighbor search less reliable when neither component provides a strong matching signal. In contrast, HoNC succeeds when paired with a strong matcher like SOSNet (50.6\%): the CNN provides robust matching, and HoNC's complementary color signal enhances discrimination rather than diluting it.
