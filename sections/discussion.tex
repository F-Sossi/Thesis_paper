...
This finding has practical implications: \textbf{keypoint selection strategy can matter as much as descriptor algorithm choice}. The 17\% gain from intersection filtering is comparable to the gap between SIFT and CNN descriptors.
...
CNN descriptors are trained on patches with photometric augmentations (brightness, contrast, gamma), leading to learned illumination invariance~\cite{mishchuk2017working,tian2019sosnet}. Traditional descriptors rely on gradient directions, which are naturally invariant to multiplicative illumination changes but not to more complex photometric transformations.
