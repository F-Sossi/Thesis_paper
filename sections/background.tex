...
Other traditional detectors include Harris corners~\cite{harris1988combined}, FAST~\cite{rosten2006machine}, and SURF~\cite{bay2006surf}, each with different trade-offs between repeatability and computational cost~\cite{lee2016survey}.
...
KeyNet was designed specifically for pairing with learned descriptors like HardNet and SOSNet, achieving state-of-the-art repeatability on HPatches~\cite{barroso2019key}.
...
\begin{itemize}
    ...
    \item \textbf{ALIKE}~\cite{zhao2023aliked}: A lightweight framework that blends handcrafted and learned methods, using a deformable transformation module to improve geometric invariance while maintaining high computational efficiency.
\end{itemize}
...
RootSIFT applies element-wise square root after L1 normalization, which is equivalent to using the Hellinger kernel~\cite{arandjelovic2012three}:
...
Learned descriptors use convolutional neural networks trained on patch correspondence tasks. While they share similar architectures (typically L2Net-style~\cite{tian2017l2net} backbones producing 128-dimensional outputs), they differ in training data and loss functions, leading to complementary learned representations.

\subsubsection{HardNet}

HardNet~\cite{mishchuk2017working} is trained on the Brown dataset (Liberty, Notre Dame, Yosemite sequences) using hard negative mining with triplet loss:

\begin{equation}
    L = \max(0, m + d(a, p) - d(a, n^-))
\end{equation}

where $(a, p)$ is a matching pair, $n^-$ is the hardest negative in the batch (selected from all non-matching pairs), and $m$ is the margin. The hard negative mining strategy forces the network to learn features that distinguish the most confusable patch pairs, producing highly discriminative descriptors. The resulting 128-dimensional descriptor is \textit{zero-centered} with values typically in [-0.3, +0.3].

\subsubsection{SOSNet}

SOSNet (Second Order Similarity Network)~\cite{tian2019sosnet} is also trained on the Brown dataset but incorporates second-order similarity constraints:
...
\subsubsection{HoNC (Histogram of Normalized Colors)}

HoNC~\cite{olson2016keypoint} computes a 128-dimensional histogram of normalized RGB values within a patch:

\begin{enumerate}
    \item Normalize each pixel's RGB values to unit sum: $(r, g, b) \rightarrow (r/(r+g+b), g/(r+g+b), b/(r+g+b))$
    \item Quantize the normalized color space into bins
    \item Accumulate weighted votes based on pixel distance from patch center
    \item L2 normalize the final histogram
\end{enumerate}

The normalization step provides some robustness to brightness changes while preserving hue information. HoNC excels at \textit{discrimination}, rejecting false matches between patches with different colors, but struggles with \textit{invariance} to illumination changes that alter apparent color (e.g., tungsten vs.\ daylight). This complementary strength makes HoNC valuable when fused with learned descriptors that provide robust matching.

\subsection{Descriptor Families Summary}

We categorize descriptors into three families based on the information they capture:

\begin{table}[h]
\centering
\caption{Descriptor families and their characteristics}
\label{tab:descriptor_families}
\begin{tabular}{llll}
\toprule
\textbf{Family} & \textbf{Examples} & \textbf{Information} & \textbf{Strengths} \\
\midrule
...
A key factor in descriptor fusion is \textit{magnitude matching}. Different descriptor families produce values at different scales before normalization:
...
HoNC~\cite{olson2016keypoint} & [0, 1] & [0, 0.3] & Normalized color histogram \\
...
\begin{itemize}
    \item Detector-descriptor pairing studies showing mismatch penalties~\cite{mikolajczyk2005comparison,ma2021local}
    \item Late fusion of matching scores
    \item Ensemble methods in image matching challenges
\end{itemize}

However, \textit{cross-detector early fusion}, averaging or concatenating descriptors from keypoints detected by different methods, remains largely unexplored. This thesis addresses this gap through our spatial intersection methodology.
...
\begin{itemize}
    ...
    \item Pre-extracted 65$\times$65 gray scale patches (original benchmark)
