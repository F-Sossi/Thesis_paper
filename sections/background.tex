...
Other traditional detectors include Harris corners, FAST~\cite{rosten2006machine}, and SURF, each with different trade-offs between repeatability and computational cost.
...
\begin{itemize}
    \item \textbf{TFeat}~\cite{balntas2016learning}: A lightweight descriptor trained with triplet loss using a shallow convolutional network.
    \item \textbf{L2-Net}~\cite{simoserra2017discriminative}: A discriminative descriptor that served as a precursor to HardNet, emphasizing L2 distance optimization in the feature space.
\end{itemize}
...
Bojanic et al.~\cite{bojanic2020comparison} define three evaluation tasks:
